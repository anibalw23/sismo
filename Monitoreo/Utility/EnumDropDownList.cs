...
namespace BootstrapEditorTemplatesNameSpace.Utility
{
    public static class EnumDropDown
    {
        ...
        public static string GetEnumDescription<TEnum>(TEnum value)
        {
            ...
            DescriptionAttribute[] description = (DescriptionAttribute[])fi.GetCustomAttributes(typeof(DescriptionAttribute), false);

            if ((description != null) && (description.Length > 0))
                return description[0].Description;
            else
            {
                DisplayAttribute[] display = (DisplayAttribute[])fi.GetCustomAttributes(typeof(DisplayAttribute), false);

                if ((display != null) && (display.Length > 0))
                    return display[0].GetName();
                else { 
